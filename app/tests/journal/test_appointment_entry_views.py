...
def test_list_all_appointments(authenticated_user, add_appointment_entry):
    ...
    url = reverse("appointment-entry-list", args=[user.slug])
...
def test_add_appointment_entry_not_current_date(authenticated_user, date_param):
    ...
    url = reverse("appointment-entry-date-list", args=[user.slug, date_param])

    res = client.post(url, appointment_data, content_type="application/json")
...
def test_get_single_appointment_entry_incorrect_id(
    ...
    url = reverse(
        "appointment-entry-detail", args=[user.slug, current_date, invalid_id]
    )
...
def test_get_all_appointment_entries_by_date(
    authenticated_user, add_appointment_entry, created_on_timestamp
):
    """
    GIVEN a Django application
...
def test_remove_appointment_entry(authenticated_user, add_appointment_entry):
    ...
    url_retrieve = reverse(
        "appointment-entry-date-list", args=[user.slug, appointment_date]
    )
...
@pytest.mark.django_db
def test_remove_appointment_invalid_id(authenticated_user):
    """
    GIVEN a Django application
...
def test_remove_appointment_not_current_date(
    authenticated_user, requested_date, add_appointment_entry
):
    """
    GIVEN a Django application
...
def test_update_appointment_entry(authenticated_user, add_appointment_entry, test_data):
    ...
    url = reverse(
        "appointment-entry-detail", args=[user.slug, current_date, appointment_entry.id]
    )
...
@pytest.mark.django_db
def test_update_appointment_entry_invalid_id(authenticated_user):
    """
    GIVEN a Django application
...
@pytest.mark.parametrize(
    "requested_timestamp",
    ["2024-07-06 17:00:00", "2023-03-05 23:15:00", "2022-09-16 06:30:00"],
)
def test_update_appointment_entry_incorrect_date(
    authenticated_user, requested_timestamp, add_appointment_entry
):
    """
    GIVEN a Django application
