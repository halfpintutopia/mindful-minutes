...
def test_get_single_improvement_entry_incorrect_id(
    ...
    """
    GIVEN a Django application
    WHEN the user requests to retrieve an improvement entry with an
    incorrect id
...
def test_get_all_improvement_entries_by_date(
    authenticated_user, add_improvement_entry, created_on_timestamp
):
    """
    GIVEN a Django application
    ...
    url = reverse("improvement-entry-date-list", args=[user.slug, date_and_time[0]])
    res = client.get(url)
...
@pytest.mark.django_db
def test_remove_improvement_entry(authenticated_user, add_improvement_entry):
    """
    GIVEN a Django application
...
@pytest.mark.django_db
def test_remove_improvement_invalid_id(authenticated_user):
    """
    GIVEN a Django application
...
def test_remove_improvement_not_current_date(
    ...
    """
    GIVEN a Django application
    WHEN the user requests to remove an improvement entry when the date is
    not today
    THEN the improvement entry is not removed
    """
    improvement_entries = ImprovementEntry.objects.all()
...
@pytest.mark.parametrize(
    ...
    [
        {
            "payload": {
                "content": "I need to listen more and talk less. Focus on "
...
def test_update_improvement_entry_incorrect_date(
    ...
    """
    GIVEN a Django application
    WHEN the user requests to update an improvement entry with an incorrect
    date
