...
def test_update_user_settings(
	...
	assert res.status_code == status.HTTP_200_OK
	assert res.data["start_week_day"] == test_data["payload"][
        "start_week_day"]
	assert res.data["morning_check_in"] == test_data["payload"][
		"morning_check_in"]
	assert res.data["evening_check_in"] == test_data["payload"][
		"evening_check_in"]
...
def test_update_user_settings_incorrect_id(
	...
	THEN the user settings are not found, and updating is not allowed,
	a 403 is returned
	"""
	client, user = authenticated_user
